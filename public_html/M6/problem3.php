...
function joinArrays($users, $activities) {
    ...
    // Then, combine user data with their activity
    foreach ($users as $user) {
        $userId = $user["userId"];
        if (isset($activityMap[$userId])) {
            $user["activity"] = $activityMap[$userId];
        } else {
            $user["activity"] = null; // Optional: fallback if activity is missing
        }
        $joined[] = $user;
    }
...
$ucid = "rsk9"; // replace with your UCID
printHeader($ucid, 3); 
?>
<table>
    <thead>
